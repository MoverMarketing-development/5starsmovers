...
import { movingSuppliesIntro, movingSuppliesItems, movingSuppliesSectionImage, servicePageMap, servicePages } from "@/lib/service-pages";
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-4 md:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          <div>
            ...
            <div className="mt-10 grid gap-5 sm:grid-cols-2">
              {movingSuppliesItems.map((item) => <article key={item.title} className="rounded-[1.6rem] border border-black/6 bg-white p-6 shadow-[0_18px_40px_rgba(18,20,23,0.08)]"><div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-[#ffdc00] text-[#121417]"><Icon name="star" className="h-5 w-5" /></div><h3 className="mt-4 font-display text-2xl font-extrabold">{item.title}</h3><p className="mt-1 text-2xl font-semibold text-[#121417]/88">{item.price}</p><p className="mt-4 text-sm leading-[1.5] text-[#121417]/64">{item.description}</p></article>)}
            </div>
          </div>
